--[[
    ...
    Collectibles - System Management (Client)

    /!\ Unless you know what you are doing, do not edit this file. /!\

    Sorry for the ugly GUI code :P
]]
...
addEventHandler("collectibles:manage", localPlayer, function(serverInfo)
    ...
    local WW, WH = 800, 600
    mainWin = guiCreateWindow((SW-WW)/2, (SH-WH)/2, WW, WH, "Collectibles - System Management", false)
    ...
    x = 10
    y = 10
    local tabCommandsScroll = guiCreateScrollPane(x, y, TW-(x*2), TH-(y*2), false, tabCommands)
    
    local PW = guiGetSize(tabCommandsScroll, false)

    settings.commands = {}

    for name, command in pairs(serverInfo.commands) do
        
        local commandLabel = guiCreateLabel(0, y, PW, 20, name..":", false, tabCommandsScroll)
        y = y + 20 + 5
        local commandEdit = guiCreateEdit(0, y, PW*(1/3), 24, command, false, tabCommandsScroll)
        y = y + 24 + 10

        settings.commands[name] = {label=commandLabel, commandEdit=commandEdit}
    end

    -- Bottom margin
    guiCreateLabel(0, y, 50, 60, " ", false, tabCommandsScroll)
    ...
    local textsInfoText = "Some texts have '%s' in them. These are replaced with certain values when the text is displayed.\nText color is in R, G, B format."
    local textsInfo = guiCreateLabel(0, y, PW, 40, textsInfoText, false, tabTextsScroll)

    y = y + 40 + 5

    settings.texts = {}

    for name, v in pairs(serverInfo.texts) do
        local text = v.text
        local r = v.r
        local g = v.g
        local b = v.b
        ...
        local textEdit = guiCreateEdit(0, y, textWidth, 24, text, false, tabTextsScroll)
        ...
        settings.texts[name] = {label=textLabel, textEdit=textEdit, rgbEdit=rgbEdit}
    end
        ...
        if source == save then
            ...
            for settingType, list in pairs(settings) do
                for name, info in pairs(list) do
                    ...
                    if settingType == "commands" then
                        updateNodes[settingType][name].attributeNames = {name=name}
                        local text = guiGetText(info.commandEdit)
                        if text == "" or text:find(" ") then
                            return showValidationError("Command for '"..name.."' cannot be empty or contain spaces")
                        end
                        updateNodes[settingType][name].value = text
                    elseif settingType == "texts" then
                        updateNodes[settingType][name].attributeNames = {name=name}
                        local text1 = guiGetText(info.textEdit)
                        if text1 == "" then
                            return showValidationError("Text for '"..name.."' cannot be empty")
                        end
                        updateNodes[settingType][name].value = text1
                        ...
                        updateNodes[settingType][name].attributes = { color = text2 }
                    elseif settingType == "types" then
                        updateNodes[settingType][name].attributeNames = {name=name}
